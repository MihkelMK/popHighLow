...
const updatePeople = (nimi) => {
    request(nimi).then((a) => {
        document.getElementsByClassName('skoor')[0].textContent = 'Skoor: ' + a['score'];
        if ('võit' in a) {
        ...
        document.getElementsByClassName('given')[0].style.cssText += `background-image: linear-gradient(rgba(0, 0, 0, 0.527),rgba(0, 0, 0, 0.5)), url(${a['given'][1][1]});`;
        document.getElementsByClassName('guess')[0].style.cssText += `background-image: linear-gradient(rgba(0, 0, 0, 0.527),rgba(0, 0, 0, 0.5)), url(${a['guess'][1][1]});`;
    });
};
